...
(() => {
  ...
  async function searchPlaces(rawQuery, submitSearch) {
    ...
    try {
      ...
      const places = data.map(placeFromNominatim);
      renderSuggestions(places);
      if (submitSearch && places[0]) selectPlace(places[0]);
    } catch (err) {
      if (err.name === 'AbortError') return;
      renderSuggestions([{ empty: true, title: 'Search is unavailable', sub: 'Try again in a moment. The map can still use GPS and saved links.' }]);
    }
  }
  ...
  function renderSuggestions(items) {
    ...
    items.forEach((place) => {
      ...
      btn.innerHTML = `
        ...
        <span class="suggestion-badge">${place.loading ? 'wait' : place.empty ? 'none' : 'Go'}</span>`;
